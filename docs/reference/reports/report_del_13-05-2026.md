# Report aggiornato del 13-05-2026
...
Questo documento sostituisce le versioni accumulative precedenti, integra in modo organico gli aggiornamenti del 30-04-2026 e mantiene solo il contenuto tecnico consolidato.
...
- **Analisi multi-file di progetto**: i blocchi disponibili in `input/` vengono indicizzati (FC/FB) e le dipendenze `CALL` vengono risolte quando il blocco chiamato è presente; se manca, viene emesso un warning diagnostico (`missing_called_blocks`) ma la generazione non viene bloccata se la topologia resta dimostrabile.
...
- `RCoil`;
- nodo `O`;
- box LAD `Move` per le scritture AWL `L ...` / `T ...`;
- box IEC `TON`, `TOF`, `CTU`.
...
- `Move` generati nella famiglia FC errata;
- enable dei `Move` collegati direttamente a `Powerrail` quando il sorgente AWL prevedeva una condizione di step o una guardia esplicita.
...
- i token step letti dal sorgente devono essere canonicalizzati prima del confronto col catalogo passi (`S0*(n)` -> `Sn`, quindi `S01` -> `S1`, `S02` -> `S2`, ecc.);
- la canonicalizzazione precede la costruzione di enable, guardie, riferimenti GRAPH e reti `Move`, così da evitare enable non intenzionali collegati direttamente a `Powerrail`;
...
- `FC 12 HMI` che popola popup, testi, bit visualizzati e scritture `Move` derivate da AWL `L ...` / `T ...`;
...
Alla data del 30-04-2026 la baseline consolidata del progetto è la seguente.
...
- `Move` derivati da AWL `L/T` assegnati alla `FC 12 HMI`, non alla `FC 13 Aux`, preservando comunque il DB owner del target scritto.
...
- canonicalizzazione obbligatoria dei token step zero-padded (`S01`, `S02`, ... -> `S1`, `S2`, ...) prima di validare catalogo step, enable, guardie e reti `Move`.
...
Alla data del 30-04-2026 il progetto ha raggiunto una baseline forte su quattro livelli:
...
La baseline include inoltre due correzioni operative ormai integrate nel modello: i `Move` da AWL `L/T` appartengono alla `FC 12 HMI` e i token step zero-padded vengono normalizzati prima della costruzione di enable e guardie.
