## Report giornaliero — YYYY-MM-DD

### Obiettivo del giorno (1–3 righe)
- 

### Input analizzati
- **File**:
- **Versione TIA/GRAPH**: TIA Portal V20 / GRAPH V2
- **Corpus usato**: target / legacy semantico / misto
- **Caso**: (AWL→IR→GRAPH/DB/FC, import, diagnosi, ecc.)

### Output prodotti (con path)
- **GRAPH FB XML**:
- **GlobalDB del pacchetto XML**:
- **FC LAD XML**:
- **Cardinalita' bundle**: `1 x FB GRAPH + N x DB + M x FC`
- **Altri artefatti**:

### Esito test in TIA (import)
- **Import FB GRAPH**: ✅/❌ (errore/nota)
- **Import GlobalDB**: ✅/❌ (commenti visibili: ✅/❌)
- **Import FC**: ✅/❌
- **Compile richiesta esplicitamente (via tia-bridge)**: ✅/❌ (JobId / errori)
- **Note**:

### Regole/conoscenza consolidata oggi
- (bullet molto sintetici; se “nuova regola”, evidenziare il perche')
- **Naming globale validato**: owner DB / branch path / leaf name ✅/❌
- **Uso tipici legacy limitato a semantica**: ✅/❌

### Diagnosi (se qualcosa non importa)
- **Ipotesi principale**:
- **Evidenza**:
- **Fix applicato / tentativo**:

### Prossimi passi (massimo 5)
- [ ] 

### Link rapidi
- **Indice docs**: `../../README.md`
- **Report consolidato**: `../../reference/reports/report_del_27-04-2026.md`
- **Specifica generatore XML**: `../../reference/specs/Specifica_master_traduzione_AWL_e_generazione_XML_TIA_V20_V2_27_04.md`
