...
def _build_graph_fb_xml(profile, ir: AwlIR, graph_topology: GraphTopology) -> str:
    ...
    return (
        ...
        f"{_render_graph_preoperations(ir)}"
        '  <Sequence>\n'
        '    <Title />\n'
        '    <Comment Informative="true">\n'
...
def _build_support_symbol_home_db_map(ir: AwlIR) -> dict[str, str]:
    ...
    # 4) Step runtime binding: step symbols (Sxx) in derived logic belong to the
    # GRAPH runtime instance DB generated by TIA (DB15.. family).
...
def _render_graph_permanent_operation_call(fc_name: str) -> str:
    fc_token = str(fc_name or "").strip()
    if not fc_token:
        return '    <PermanentOperation ProgrammingLanguage="LAD" />'
    return (
        '    <PermanentOperation ProgrammingLanguage="LAD">\n'
        '      <FlgNet>\n'
        '        <Parts>\n'
        '          <Call UId="21">\n'
        f'            <CallInfo Name="{escape(fc_token)}" BlockType="FC" />\n'
        '          </Call>\n'
        '        </Parts>\n'
        '        <Wires>\n'
        '          <Wire UId="22">\n'
...
def _render_graph_preoperations(ir: AwlIR) -> str:
    """
    Emit PermanentOperation calls for the support FCs.

    This matches the validated export pattern where DIAG/HMI/AUX/LEV2/OUTPUT/TRANSITIONS
    FCs are called cyclically so the bundle is functional right after import.
    """
    _, diag_fc_name, _, _, _, _ = _support_block_names(ir.sequence_name, "diag")
    _, hmi_fc_name, _, _, _, _ = _support_block_names(ir.sequence_name, "hmi")
    _, aux_fc_name, _, _, _, _ = _support_block_names(ir.sequence_name, "aux")
    _, mode_fc_name, _, _, _, _ = _support_block_names(ir.sequence_name, "mode")
    _, output_fc_name, _, _, _, _ = _support_block_names(ir.sequence_name, "output")
    _, tr_fc_name, _, _, _, _ = _support_block_names(ir.sequence_name, "transitions")

    rendered_calls = "\n".join(
        _render_graph_permanent_operation_call(name)
        for name in [diag_fc_name, hmi_fc_name, aux_fc_name, mode_fc_name, output_fc_name, tr_fc_name]
        if name
    )
    if not rendered_calls:
        rendered_calls = '    <PermanentOperation ProgrammingLanguage="LAD" />'
    return f"  <PreOperations>\n{rendered_calls}\n  </PreOperations>\n"
...
def _derive_awl_action_logic_rows(ir: AwlIR) -> dict[str, list[dict[str, object]]]:
    ...
    def _map_symbol(raw: str, network_index: int) -> str:
        normalized = _normalize_operand_token(raw)
        # AWL locals (Lxx.y) are stack/local intermediates; do not export them as global tags.
...
def _collect_condition_logic(network: AwlNetwork) -> tuple[str, list[str]]:
    ...
    # Drop AWL local-temporary flags (Lxx.y) from exported logic:
    # they are stack/local intermediates, not global PLC tags.
    # Keeping them would create nonsense global symbols in FC/GRAPH conditions.
    drop = {op for op in operands if re.fullmatch(r"L\\d+(?:\\.\\d+)?", str(op or "").strip(), flags=re.IGNORECASE)}
    if drop:
        operands = [op for op in operands if op not in drop]
        if expression:
...
def _parse_condition_expression(
    ...
    while index < len(tokens):
        ...
        if kind == "atom":
            ...
            # Keep step operands as part of the condition expression.
            # They are later rebound to the GRAPH runtime DB (e.g. S10 -> DB15..S10.X)
            # so FC output logic can be gated by the active step.
            term_ops = [operand]
