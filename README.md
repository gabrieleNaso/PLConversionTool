...
Il contesto tecnico consolidato e' integrato direttamente in questo `README`, che ora e' il documento unico di riferimento del progetto. [Contesto_progetto.md](/home/administrator/PLConversionTool/Contesto_progetto.md) resta come alias storico.

## Da dove partire
- [Indice documentazione](/home/administrator/PLConversionTool/docs/INDEX.md)
- `README.md` di questa root come documento unico di contesto e stato progetto
...
- l'import remoto di un XML salvato sulla VM Ubuntu verso `TIA Portal` sulla VM Windows e' stato verificato con successo;
- l'export remoto di un XML da `TIA Portal` sulla VM Windows verso la VM Ubuntu e' stato verificato con successo;
...
- il job `import` remoto tramite `tia-bridge`, con `artifactPath` Linux e staging automatico su Windows, e' stato provato con esito positivo;
- il job `export` remoto tramite `tia-bridge`, con sync automatica del risultato da Windows a Ubuntu, e' stato provato con esito positivo;
...
- un XML presente in `output/` sulla VM Ubuntu puo' essere importato in TIA senza copia manuale preventiva sulla VM Windows;
- un XML esportato da TIA sulla VM Windows puo' essere riportato automaticamente in `output/` sulla VM Ubuntu;
...
- espone il trasferimento file verso Windows per staging degli artefatti di import;
- espone lettura e listing file per sincronizzare verso Ubuntu i risultati degli export remoti;
- espone `GET /api/jobs`;
...
- riceve XML dal bridge Linux e li salva in `TempDirectory\\bridge_uploads\\...` prima dell'import remoto;
- salva gli export remoti in `TempDirectory\\bridge_exports\\...` prima della sync di ritorno verso Ubuntu;
...
- staging automatico degli XML dalla VM Ubuntu alla VM Windows per import remoto;
- sincronizzazione automatica degli XML esportati da Windows verso la VM Ubuntu;
...
## Contesto Tecnico Consolidato

Il contenuto seguente integra integralmente il precedente `Contesto_progetto.md` e diventa parte del documento unico di progetto.
